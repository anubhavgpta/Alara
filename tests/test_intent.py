...
Contains 50 real developer commands that developers would actually say.
...
# Now import from alara modules
from core.intent_engine import IntentEngine, Action
...
# 50 real developer commands across different categories
TEST_CASES = [
    # App Control (10)
...
class IntentTestSuite:
    ...
    def run_test_case(self, test_case: TestCase) -> Tuple[bool, Action]:
        """Run a single test case and return (passed, actual_action)"""
        try:
            ...
            # For unknown actions, just check the action type
            if test_case.expected_action == "unknown":
                passed = actual.action == "unknown"
            else:
                # For known actions, check both action and key params
                passed = action_match
                if passed and test_case.expected_params:
                    for key, expected_value in test_case.expected_params.items():
                        ...
                        # Convert to string to handle integer params (e.g., amount: 10)
    ...
    def export_results(self, results: Dict[str, any], filename: str = "test_results.json"):
...
def main():
    ...
    if accuracy >= 90:
        ...
        print(f"\n⚠️  NEEDS WORK: {accuracy:.1f}% accuracy is below the 90% target")
    else:
