    ...
    def _build_approach(self, goal_context: GoalContext, memory_context: MemoryContext | None) -> str:
        ...
        try:
            ...
            approach_system_prompt = (
                ...
                "  browser: navigate, click, type, scrape, screenshot, fill_form, submit_form, get_links, wait_for, extract_table, search_web\n"
                "  comms: send_email, read_emails, create_calendar_event, get_calendar_events, send_slack_message, create_notion_page, update_notion_page, create_task, send_whatsapp, trigger_webhook\n\n"
...
"""
        ...
        return agent_section + (
            ...
            "  scrape            params: { url, selector }\n"
            "  screenshot         params: { url, path }\n"
            "  fill_form         params: { url, fields }\n"
            "  submit_form       params: { url, fields, submit_selector }\n"
            "  get_links         params: { url, filter }\n"
            "  wait_for          params: { url, selector, timeout_ms }\n"
            "  extract_table     params: { url, selector }\n"
            "  search_web        params: { query, engine }\n"
            "Comms:\n"
            "  send_email        params: { to, subject, body }\n"
            "  read_emails       params: { limit }\n"
            "  create_calendar_event params: { title, start_time, end_time, attendees }\n"
            "  get_calendar_events params: { start_date, end_date }\n"
            "  send_slack_message params: { channel, text }\n"
            "  create_notion_page params: { title, content, parent_id }\n"
            "  update_notion_page params: { page_id, content }\n"
            "  create_task       params: { title, description, due_date }\n"
            "  send_whatsapp     params: { to, message }\n"
            "  trigger_webhook   params: { url, method, data }\n\n"
            ...
            "- browser operations -> preferred_layer: browser\n"
            "- comms operations -> preferred_layer: comms\n\n"
            "Fallback strategy:\n"
